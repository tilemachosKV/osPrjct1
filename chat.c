...
void init(int argc, char *argv[])
{
  ...
  sem_init(&someMemory->shm_sem, 1, 0);
  someMemory->active_dialogues_count++;
  ...
  while (strncmp(user_input, "DONE", 5) != 0)
  { // Use 5 to check "DONE\0"
    printf("NEW MESSAGE: ");
    fflush(stdout);

    int scan_result = scanf("%127s", user_input);
    if (scan_result != 1)
    {
    ...
    strcpy(someMemory->dialogues[0].message.text, user_input);
...
void attach(int argc, char *argv[])
{
  ...
  while (strncmp(someMemory->dialogues[0].message.text, "DONE", 4) != 0)
  {
    sem_wait(&someMemory->dialogues[0].message.access);
    printf("MESSAGE: '%s'\n", someMemory->dialogues[0].message.text);
  }
